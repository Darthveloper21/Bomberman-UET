...
void player2::update2(float delta, const Uint8 *keyState, char status_map[][100], char bomb_map[][100], char power_map[][100])
{
    ...
    if(bombtime - initbomb >= waittime && explode == true)
    {
        ...
        for(int i = 0; i <= bomb_length2; ++i) {if(status_map[bx + i][by] != '2' && bx + i < 14) bomb_map[bx + i][by] = 'f'; else break;}
        for(int i = 0; i <= bomb_length2; ++i) {if(status_map[bx - i][by] != '2' && bx - i > 0)  bomb_map[bx - i][by] = 'f'; else break;}
        for(int i = 0; i <= bomb_length2; ++i) {if(status_map[bx][by + i] != '2' && by + i < 17) bomb_map[bx][by + i] = 'f'; else break;}
        for(int i = 0; i <= bomb_length2; ++i) {if(status_map[bx][by - i] != '2' && by + i > 0)  bomb_map[bx][by - i] = 'f'; else break;}
